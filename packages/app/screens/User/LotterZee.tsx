...
export default function UserLotterZeeScreen() {
  const nav = useNavigation();
  const [size, onLayout] = useComponentSize();
  const route = useRoute<RouteProp<UserStackParamList, "LotterZee">>();
    ...
    <Layout onLayout={onLayout} style={{ flex: 1, flexDirection: "row" }}>
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 4 }}>
        {data.data.data.balls.length > 0 ? (
          <>
            <Text category="h4">You have captured {data.data.data.balls.length} LotterZEE Balls</Text>
            <Input value={data.data.data.balls.map(i => i.pin_icon.slice(62, -4)).join("")} label="Sequence" accessoryRight={props => <Button onPress={() => {
              ExpoClipboard.setString(data.data.data.balls.map(i => i.pin_icon.slice(62, -4)).join(""));
            }} {...props} appearance="outline" style={{ margin: -8 }} accessoryLeft={props => <Icon {...props} name={"content-copy"} />} />} />
          </>
        ) : (
