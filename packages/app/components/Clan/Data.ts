...
export function ClanStatsConverter(
  ...
  // for (const level of Object.keys(requirements.data.levels)
  //   .map((i) => Number(i))
  //   .sort((a, b) => b - a)) {
  //   // Loop through individual requirement
  //   for (const requirement of requirements.data.levels[level].individual) {
  //     // Make sure requirement included in lists
  //     if (!data.all.includes(requirement.task_id))
  //       data.all.push(requirement.task_id);
  //     if (!data.individual.includes(requirement.task_id))
  //       data.individual.push(requirement.task_id);
  //     let increment = false;
  //     if (!data.requirements[requirement.task_id]) {
  //       increment = true;
  //       data.requirements[requirement.task_id] = {
  //         value: 0,
  //       };
  //     }
  //     for (const user in requirement.data) {
  //       if (!data.users[user])
  //         data.users[user] = {
  //           username: null,
  //           user_id: Number(user),
  ...
  //           level: 5,
  //         };
  //       data.users[user].requirements[requirement.task_id] = {
  //         value: requirement.data[user],
  //         level:
  //           requirement.data[user] >= requirement.amount
  //             ? Math.max(
  //                 data.users[user].requirements[requirement.task_id]?.level ??
  //                   0,
  //                 Number(level)
  //               )
  //             : Number(level) - 1,
  //       };
  //       data.users[user].level = Math.min(
  //         data.users[user].level,
  //         data.users[user].requirements[requirement.task_id].level || 0
  //       );
  //       if (increment)
  //         data.requirements[requirement.task_id].value +=
  //           requirement.data[user];
  //     }
  //   }
  //   for (const requirement of requirements.data.levels[level].group) {
  //     if (!data.all.includes(requirement.task_id))
  //       data.all.push(requirement.task_id);
  //     if (!data.group.includes(requirement.task_id))
  //       data.group.push(requirement.task_id);
  //     data.requirements[requirement.task_id] = {
  //       value: requirement.value,
  //       level:
  //         requirement.value >= requirement.amount
  //           ? Math.max(
  //               data.requirements[requirement.task_id]?.level ?? 0,
  //               Number(level)
  //             )
  //           : Number(level) - 1,
  //     };
  ...
  //           level: 0,
  //         };
  //       if (!data.users[user].requirements[requirement.task_id]) {
  //         data.users[user].requirements[requirement.task_id] = {
  //           value: requirement.data[user],
  //         };
  //       }
