...
export const gray_light: Theme = generateLightTheme(
  ...
  c => {
    let x = color(c);
    return x.hue((x.hue() || 220) + 25).saturationv(0).value(0);
  },
  c => {
    let x = color(c);
    return x.hue((x.hue() || 220) + 25).saturationv(0).darken(0.8);
  },
  "light"
)
...
function generateTheme(base: Record<string, string>, a: (c: string, l: string) => color, b: (c: string, l: string) => color, style: "dark" | "light"): Theme {
  return {
    ...Object.fromEntries(Object.entries(base).map(i=>{
      if(i[0].match(/^color-basic-[1-6]00$/) || i[0].match(/^color-basic-transparent-[1-6]00$/)) {
        return [i[0], a(i[1], i[0]).rgb().string()];
      } else if(i[0].match(/^color-basic-[7-9]00$/) || i[0].match(/^color-basic-1[0-1]00$/) || i[0].match(/^color-primary-[0-9]+$/) || i[0].match(/^color-primary-transparent-[0-9]+$/)) {
...
function generateLightTheme(base: Record<string, string>, a: (c: string, l: string) => color, b: (c: string, l: string) => color, c: (c: string, l: string) => color, d: (c: string, l: string) => color, style: "dark" | "light"): Theme {
  return {
    ...Object.fromEntries(Object.entries(base).map(i=>{
      ...
      } else if(i[0].match(/^color-primary-[0-9]+$/) || i[0].match(/^color-primary-transparent-[0-9]+$/)) {
