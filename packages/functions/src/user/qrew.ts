  ...
  versions: [
    {
      ...
      async function({
        ...
        const formattedArchived = archived.map(i => i.capture_type_id).reduce((obj, item) => {
        ...
        var recent_cap = Object.entries(capture_dates?.data ?? {}).sort((a, b) => new Date(b[0]).valueOf() - new Date(a[0]).valueOf())[0][0];
        var recent_dep = Object.entries(deploy_dates?.data ?? {}).sort((a, b) => new Date(b[0]).valueOf() - new Date(a[0]).valueOf())[0][0];
        var this_month = day().tz('America/Chicago').date() < 15;
        var next_check = day().tz('America/Chicago').add(this_month ? 0 : 1, "month").date(this_month ? 15 : 1).hour(0).minute(0).second(0).millisecond(0);
